...
public class CordinatesFinderHotels {
    public boolean findedForHotels = false;
    private void SearchText(TextView resultView) {
        new Handler(Looper.getMainLooper()).post(() -> resultView.setText("Searching for hotels..."));
    }

    public void getHotelCoordinates(double userLat, double userLng, int radius, String apiKey, TextView resultView, GridLayout resultsContainer) {
                ...
                response -> {
                    try {
                        ...
                        if (results.length() > 0) {
                            ...
                            updateResultView(resultView, "No hotels found within the radius.");
                        }
                    } catch (Exception e) {
