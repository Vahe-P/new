...
public class CoordinatesFinderLibraries {
    ...
    public boolean findedForLibrariess=false;
    ...
    public void getLibraryCoordinates(double userLat, double userLng, int radius, String apiKey, TextView resultView, RecyclerView resultsContainer) {
                ...
                response -> {
                    try {
                        ...
                        if (results.length() > 0) {
                            ...
                            // Collect all results in a single loop
                            for (int i = 0; i < results.length(); i++) {
                                ...
                                // Log for debugging
                                Log.e("Library", "Processing church: " + place.getString("name"));
                            }

                            // Pass all collected data to getStreetDistances
                            getStreetDistances(resultView.getContext(), userLat, userLng, destinations.toString(), apiKey, coordinates, results, resultView, resultsContainer, radius);
                        } else {
    ...
    private void getStreetDistances(Context context, double userLat, double userLng, String destinations, String apiKey,
                                    StringBuilder coordinates, JSONArray results, TextView resultView, RecyclerView container, int radius) {
        ...
        JsonObjectRequest distanceRequest = new JsonObjectRequest(Request.Method.GET, distanceUrl, null,
                response -> {
                        ...
                        if (rows.length() > 0) {
                            ...
                            for (int i = 0; i < elements.length(); i++) {
                                ...
                                if (element.getString("status").equals("OK")) {
                                    ...
                                    // Parse the numeric part of the distance
                                    String numericPart = distanceText.split(" ")[0];
                                    float distanceInKm = Float.parseFloat(numericPart);

                                    // Add the place to the list only if within the specified radius
                                    if (nameChecker(name) && distanceInKm <= radius) {
                                        Log.e("KLLL", "Adding church: " + name);
                                        places.add(new Place_2(name, photoUrl, lat, lng, distanceText,"Library"));
                                    }
                                } else {
                    ...
                    updateResultView(resultView, findedForLibrariess ? "Filtered Results:" : "No churches found within the radius");
                },
